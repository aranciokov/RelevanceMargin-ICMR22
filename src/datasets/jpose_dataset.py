...
from datasets.mmen_dataset import MMEN_Dataset, create_epic_mmen_dataset, create_youcook_mmen_dataset
...
def create_epic_jpose_dataset(is_train=True, batch_size=EPIC_MMEN.batch_size, num_triplets=EPIC_MMEN.num_triplets, is_test=False,
                              all_nouns=False, rgb_only=False, rgb_flow_only=False):
    verb_ds = create_epic_mmen_dataset('verb', is_train=is_train,
            batch_size=batch_size, num_triplets=num_triplets, action_dataset=False, is_test=is_test,
                                       rgb_only=rgb_only, rgb_flow_only=rgb_flow_only)
    noun_ds = create_epic_mmen_dataset('noun', is_train=is_train,
            batch_size=batch_size, num_triplets=num_triplets, action_dataset=False, is_test=is_test,
                                         all_nouns=all_nouns, rgb_only=rgb_only, rgb_flow_only=rgb_flow_only)
...
def create_youcook_jpose_dataset(is_train=True, batch_size=EPIC_MMEN.batch_size, num_triplets=EPIC_MMEN.num_triplets, is_test=False,
                              all_nouns=False, rgb_only=False, rgb_flow_only=False):
    verb_ds = create_youcook_mmen_dataset('verb', is_train=is_train,
            batch_size=batch_size, num_triplets=num_triplets, action_dataset=False, is_test=is_test,
                                       all_nouns=all_nouns, rgb_only=rgb_only, rgb_flow_only=rgb_flow_only)
    noun_ds = create_youcook_mmen_dataset('noun', is_train=is_train,
            batch_size=batch_size, num_triplets=num_triplets, action_dataset=False, is_test=is_test,
                                         all_nouns=all_nouns, rgb_only=rgb_only, rgb_flow_only=rgb_flow_only)
    action_ds = create_youcook_mmen_dataset('caption', is_train=is_train,
            batch_size=batch_size, num_triplets=num_triplets, action_dataset=True, is_test=is_test,
                                         all_nouns=all_nouns, rgb_only=rgb_only, rgb_flow_only=rgb_flow_only)
    return JPOSE_Dataset(verb_ds, noun_ds, action_ds, batch_size=batch_size, num_triplets=num_triplets)
