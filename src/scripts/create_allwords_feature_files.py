...
def create_sentence_only_vector(model, sentence, type_='average', split_str=' '):
    ...
    sentence_vector = np.array([get_word_embedding(model, w) for w in words])
...
def create_noun_only_vector(model, nouns, type_='average'):
    ...
    sentence_vector = np.array([get_word_embedding(model, n) for n in final_nouns])
    return sentence_vector
...
def main(args):
    ...
    if args.caption:
        save_output_pickle(caption_features, args.out_dir, datafame_name, 'caption_allwords')
